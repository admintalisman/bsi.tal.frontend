...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="TEMPBSICIT_verifikasi" && $field=="CIT_FL")
	{
		if($value == 0){
	$value = "Proses";
}elseif($value == 1){
	...
	$value = "Draft";
		...
		return $value;
				...
				if($table=="TEMPBSICIS_KC" && $field=="CIS_FL")
	{
		if ($value == 0){
	$value = "Sukses Terupload";
}elseif ($value == 1){
	$value = "Sukses Diproses";
}elseif ($value == 2){
	$value = "Overlimit";
}
;
		return $value;
				...
				if($table=="TEMPBSICIS_ATM" && $field=="CIS_FL")
	{
		if ($value == 0){
	$value = "Sukses Terupload";
}elseif ($value == 1){
...
;
		return $value;
				...
				if($table=="TEMPBSICIS_OL" && $field=="CIS_FL")
	{
		if ($value == 0){
	$value = "Sukses Terupload";
}elseif ($value == 1){
	...
	$value = "Overlimit";
		...
		return $value;
				...
				if($table=="TEMPBSICIT_COLL" && $field=="CIT_FL")
	{
		if($value == 0){
	$value = "Proses";
}elseif($value == 1){
	...
	$value = "Draft";
		...
		return $value;
				...
				if($table=="TEMPBSICIT_COLL" && $field=="IS_OL")
	{
		if($value == 1){
	$value = "Overlimit";
}else{
	$value = "";
		...
		return $value;
				...
				if($table=="TEMPBSICIT_CIT" && $field=="CIT_FL")
	{
		if($value == 0){
	$value = "Proses";
}elseif($value == 1){
	...
	$value = "Draft";
		...
		return $value;
				...
				if($table=="TEMPBSICIT_CIT" && $field=="IS_OL")
	{
		if($value == 1){
	$value = "Overlimit";
}else{
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="TEMPBSICILE" && $field=="ADDUSER")
	{
		return $_SESSION["UserName"];
	}
				if($table=="TEMPBSICILE" && $field=="ADDDATE")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="TEMPBSICIT" && $field=="CIT_REGDATE")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="TEMPBSICIT" && $field=="CIT_MTU")
	{
		return "MTU";
	}
				if($table=="TEMPBSICIT" && $field=="CIT_EDIT_USER")
	{
		return $_SESSION["UserID"];
	}
				if($table=="TEMPBSICIT" && $field=="CIT_EDIT_DATE")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="TEMPBSICIT" && $field=="CIT_FL")
	{
		return 0;
	}
				if($table=="TEMBSICITASAL" && $field=="CIT_ASAL_MTU")
	{
		return "IDR";
	}
				if($table=="TEMPBSICIS" && $field=="CIS_UPL_USER")
	{
		return $_SESSION["UserID"];
	}
				if($table=="TEMPBSICIS" && $field=="CIS_UPL_DATE")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="TEMPBSICIS" && $field=="CIS_FL")
	{
		return 0;
	}
				if($table=="TEMPBSICIS_view" && $field=="CIS_UPL_USER")
	{
		return $_SESSION["UserID"];
	}
				if($table=="TEMPBSICIS_view" && $field=="CIS_UPL_DATE")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="TEMPBSICIS_view" && $field=="CIS_FL")
	{
		return 0;
	}
				if($table=="TEMBSICITASAL_view" && $field=="CIT_ASAL_MTU")
	{
		return "IDR";
	}
	return "";
