...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		$sql = "update ". $this->connection->addTableWrappers( Security::loginTable() )
			." set ". $this->connection->addFieldWrappers( "˂Create new˃" )."=1 "
			." where " .
			$this->connection->comparisonSQL(
				$this->getFieldSQLDecrypt(Security::usernameField()),
				$strUsername,
	...
	protected function checkRegisterData( $strUsername, $strPassword, $strEmail )
	{
		...
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Username tidak boleh kosong";
			$ret = false;
		}
		else if( !$this->checkIfUsernameUnique( $strUsername ) )
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Username"." <i>".runner_htmlspecialchars( $strUsername )."</i> "."sudah terpakai. Pilih username lainnya.";
			$ret = false;
		}
		...
		if( Security::emailField() && $this->pSet->appearOnPage( Security::emailField() ) )
		{
			...
			if( !strlen($strEmail) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Mohon masukan kembali alamat email yang sah";
				$ret = false;
			}
			else if( !$this->checkIfEmailUnique( $strEmail ) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Email"." <i>". runner_htmlspecialchars( $strEmail )."</i> "."telah terdaftar. Bila anda lupa username atau password anda, gunakan formulir pengingat password";
				$ret = false;
			}
		}
