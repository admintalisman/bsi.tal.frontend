...
class LoginPage extends RunnerPage
{
	...
	protected function prepareTwoFactorMessage() {
		...
		if( $destination["method"] === "phone" ) {
			$twofMessage = str_replace( "%phone%", $destination["address"], "Sebuah pesan teks dengan kode Anda telah dikirim ke: %phone%" );
		} else if( $destination["method"] === "email" ) {
			$twofMessage = str_replace( "%email%", $destination["address"], "Sebuah email dengan kode Anda telah dikirim ke %email%." );
		} else if( $destination["method"] === "totp" ) {
			$twofMessage = str_replace(
				array( "%username%", "%site%" ),
				array( "<br><b>".Security::provisionalUsername()."</b>", "<b>".$destination["address"]."</b>" ),
				"Masukkan kode dari aplikasi otentikasi Anda sesuai dengan %username% di %site%."
			);
			$this->hideItemType("resend_button");
		}
